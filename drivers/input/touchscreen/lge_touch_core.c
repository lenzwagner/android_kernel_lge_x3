...
#ifdef MINFREQ_FOR_TS
static struct pm_qos_request cpu_freq_min_req;

static inline void cpufreq_set_min_freq(s32 frequency)
{
	pm_qos_update_request(&cpu_freq_min_req, frequency);
}
#endif
...
#ifdef MINFREQ_FOR_TS
		cpufreq_set_min_freq((s32)NOLAG_FREQ);
#endif
		break;
	case ABS_RELEASE:
		input_mt_sync(ts->input_dev);
