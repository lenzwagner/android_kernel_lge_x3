...
void rawchip_release(void)
{
	...
	free_irq(gpio_to_irq(pdata->rawchip_intr0), 0);
	free_irq(gpio_to_irq(pdata->rawchip_intr1), 0);
...
int rawchip_open_init(void)
{
	...
	/*create irq*/
	rc = request_irq(gpio_to_irq(pdata->rawchip_intr0), yushan_irq_handler,
	...
	rc = request_irq(gpio_to_irq(pdata->rawchip_intr1), yushan_irq_handler2,
