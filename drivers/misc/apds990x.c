				   ...
				   const struct i2c_device_id *id)
{
	...
	data->is_white = 0;			// white after Rev.D
        ...
        pr_info("[SENSOR-APDS]%s : get cross-talk = %d , ppcount_val = %d \n",__func__,cross_talk_val, ppcount_val);
	...
	pr_info("[SENSOR-APDS] apds990x_probe enable(%d) = %d\n", data->enable, APDS990x_IRQ);
	...
	pr_info("[SENSOR-APDS]%s prior data->enable_als_sensor = %d LINE[%d]\n",__func__,data->enable_als_sensor,__LINE__);
	pr_info("[SENSOR-APDS]%s support ver. %s enabled\n", __func__, DRIVER_VERSION);
